...
fa/                       # 代码
  ...
  note_template.py        # 12+3 维笔记模板（12 基础 + 3 增强维，fa report 的 general 保底同源）
  ...
  framework/              # L1 硬框架（人改）
    ...
    frameworks/           # 分析框架注册表（fa report 路由用，git 跟踪）
      <name>.md           # 一框架一文件：frontmatter(name/title/description/applies/avoid)+正文
                          # 人写人改可持续外挂；路由闭合词表=注册表本身，选不出回退 12+3 维 general
...
| **Tier 1.12** | fa report 个股研究笔记（愿景第二步）：vet 校验 + 框架路由 + 框架分析 → Word；分析框架注册表 `memory/framework/frameworks/`（注册表即闭合词表，LLM 只能选已存在框架，选不出回退 12+3 维 general）；首批移植 leopold/serenity/imacompnerd 三框架（近原文搬运，数据来源适配为快照+知识库，缺数据标「待人工补查」）；极简 md→docx 渲染器 | ✅ |
