...
定位：**研究合成助手**——用逻辑确定方向、用基本面校验空间的个股逻辑分析工具。

1. **长期记忆**：CoT（行业可复用逻辑）+ note（个股论点）+ 闭合词表召回，三层记忆（硬框架/软知识/情景）
2. **知识合成（主线）**：`fa vet`（单股/观点-only/批量校验）+ `fa report`（框架路由研究笔记 → Word）
3. **方便输入**：PDF/PPT/Excel/Word/md 研报 + 用户论点，agent 形成与用户一致的思考逻辑
4. **进化 = 点评沉淀**：用户对输出的长期点评 → `memory/framework/review-rules.md` → 注入合成提示词（人审入库）。
   原「Predictor → Critic → Reflector → Evolver」预测进化闭环已冻结（2026-06-10，代码保留不投入，见 ROADMAP 冻结区）——
   市场反馈太稀疏，校准不出概率；点评反馈密集即时，是可行的进化燃料
...
fa/                       # 代码
  ...
  framework.py            # 硬框架加载/更新（读写 memory/framework/）+ 分析框架注册表（frameworks/）
  vet.py                  # 逻辑校验器（fa vet）：单股/观点-only/批量清单，用已有 CoT+note 校验
  report.py               # 个股研究笔记（fa report）三段式：vet 校验 + 框架/9维材料分析 + 统一估值预期 → Word
  docx_render.py          # 极简 markdown → docx 渲染器（report 输出用）
  sectors.py              # 板块清单 + 同义词归一 + 链级主题分类（classify_chains）
  note_extractor.py       # 文档/文本 → canonical_15d_v1 稀疏个股 note（一报告一 note）
  note_template.py        # canonical_15d_v1 笔记模板（15 维稀疏结构，fa report 的 general 保底同源）
  consolidate.py          # 多份 report-level notes → company-level synthesis + 冲突清单
  review_v2.py            # 基于 canonical note frontmatter 的结构化复盘
...
docs/                     # 项目文档
  ...
  DATA_SOURCES.md         # 本机 DB/外部数据源盘点；研报/公告/基本面数据边界
  USAGE.md                # 日常使用命令速查
...
memory/                   # 持久化数据（git 管理，可读、可追溯）
  framework/              # L1 硬框架（人改）
    ...
    red-flags.md          # 风险信号库
    valuation.md          # 估值方法论
    review-rules.md       # 点评沉淀：用户对 vet/report 输出的长期方法论规则（人审入库；
                          # `## 规则` 后正文注入合成提示词，一次性纠错不进这里；~30 条时合并修剪）
    frameworks/           # 分析框架注册表（fa report 路由用，git 跟踪）
      <name>.md           # 一框架一文件：frontmatter(name/title/description/applies/avoid)+正文
                          # 人写人改可持续外挂；路由闭合词表=注册表本身，选不出回退 canonical 15 维 general
                          # `_` 开头跳过（草稿/停用）；外部技能移植时近原文搬运，只适配数据来源
  ...
  theses/                 # 个股论点
    user/                 # 用户写的论点，召回权重 2.0
      ...
      _raw/               # fa note -f 原始文件归档；note frontmatter raw_path 指向这里
        text/             # fa note -f 抽文文本归档；raw_text_path 指向这里
    company/              # company-level synthesis：同一公司多份 report-level notes 的可重算综合稿
      <ticker>_<yyyy-mm-dd>.md
    conflicts/            # fa consolidate 产出的冲突/过时信息 JSONL，可重算
      <ticker>_<yyyy-mm-dd>.jsonl
...
- **note 分两层**：`memory/theses/user/` 是 report-level evidence slice，一份报告一份 note，禁止自动覆盖、删除或把冲突抹平；`memory/theses/company/` 是 company-level synthesis，可由 `fa consolidate <ticker>` 重算。综合稿要引用来源 note，冲突写进 `memory/theses/conflicts/`。
- **canonical note 稀疏填写**：`template_version/note_schema = canonical_15d_v1`。15 个维度允许缺失；报告没有证据就留空/写暂无证据，不为了填满而编造。
- **note 原文可追溯**：`fa note -f` 必须写 `source_doc/source_hash/raw_path/raw_text_path`。`raw_path` 指原始文件，`raw_text_path` 指抽文文本；`fa note --reextract` 优先从原始文件重抽，原件缺失时回退抽文文本。
- **Wind DB 数据源边界**：新接数据前先看 `docs/DATA_SOURCES.md`。`reportdata.report_info` 是研报索引/摘要库，无 PDF/URL，`content` 样本基本为空；`financedata.ashareanninf` 有 A股公告 HTML 正文或 PDF 链接，适合查治理/减持增持/业绩预告/订单/产能等三表外信息。
...
加新功能/改 storage 后还要跑：
```bash
fa vet 2513.HK --no-save        # 端到端冒烟（DeepSeek 真实调用，召回+合成两跳）
# 动了 report/框架路由相关代码 → 再跑 fa report 2513.HK
# （fa deep 已降级为快速分析，不再是冒烟命令——进化闭环 2026-06-10 冻结）
```
...
| **Tier 1.11** | vet 三入口：观点-only 模式（无 ticker、按主题召回 + `--tag` 闭合词表过滤 + 标的映射）+ `--batch` 批量轻量扫描（xlsx/csv/txt/内联清单 → 每股 1 次 LLM 调用 → 汇总+命中明细两 sheet Excel；CoT 目录放前缀吃 DeepSeek 缓存）；顺修 vet 召回 max_tokens 被 thinking 吃光致 JSON 必败（1500→4000） | ✅ |
| **Tier 1.12** | fa report 个股研究笔记（愿景第二步）：vet 校验 + 框架路由 + 框架分析 → Word；分析框架注册表 `memory/framework/frameworks/`（注册表即闭合词表，LLM 只能选已存在框架，选不出回退 canonical 15 维 general）；首批移植 leopold/serenity/imacompnerd 三框架（近原文搬运，数据来源适配为快照+知识库，缺数据标「待人工补查」）；极简 md→docx 渲染器 | ✅ |
| **Tier 1.13** | 点评进化第一步：`review-rules.md` 注入 vet/report 合成（空文件跳过、batch 循环外算保缓存）+ 预测进化闭环正式冻结（Mem-Palace/GEPA 解锁数据断供=事实死亡；embedding 成唯一活触发器；`fa deep` 降级快速分析；详见 ROADMAP 冻结区） | ✅ |
| **Tier 1.14** | report 三段式重排（点评驱动）：第二部分 = 专用框架或通用 9 维材料分析（核心论点/业务/财务质地/格局/护城河/管理层/成长史/风险/竞争评级），第三部分 = **统一**估值与预期分析（盈利预测/估值目标价/催化剂/反证/跟踪指标/待人工补查清单）；数据层加 `fetch_forecast_pack`（EODHD 历史利润表 + 卖方一致预期 Trend + 目标价评级，A/港股也有 Trend）；client 全局 max_retries=4（5 连环调用防瞬断） | ✅ |
| Tier 2 | **fa chat 体验升级（rich UI + 上下文裁剪/会话持久化 + search/get 召回 + 软删除/合并）✅** ; Web UI / 微信 bot / 多 workspace ⏳ | 🔵 进行中 |
| Tier 3 | embedding 补充召回（活）+ 多模态（活）；Mem-Palace / GEPA / 单链回测 → 已冻结（ROADMAP 冻结区有复活条件） | ⏳ |
