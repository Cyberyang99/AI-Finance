"""从一份文档/一段文本中抽 12 维度个股投资逻辑 note.

两步：
  1. is_individual_research(doc_text) — LLM 快速判断是否针对某具体股票的深度研究
  2. extract_12d(doc_text, ticker, comment) — LLM 抽 12 维度
...
EXTRACT_SYSTEM = "你是资深股票研究员。从一份个股投资资料中抽出 12 个维度的结构化投资逻辑，写不出来的字段留空。"

EXTRACT_TEMPLATE = """从下面这份针对 **{ticker}** 的深度研究资料中，抽出 12 维度的投资逻辑。
...
## 12 维度（每个都简明扼要，能填则填，不能则留空字符串/空数组）
...
3. **指标单位用「亿元」**：revenue_yi=收入(亿元), profit_yi=利润(亿元), mcap_yi=市值(亿元)
4. **百分比用小数**：net_margin=0.22 (不是 22)，share_pct=0.30 (不是 30)
5. **业务拆解保留细节**：business_breakdown 里把每条业务线的占比/客户/景气分别写到一行
6. **风险条目写量化下跌空间**：能算就算（X% 下跌）
