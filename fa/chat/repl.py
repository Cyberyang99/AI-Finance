...
8. **改正/纠错用 edit_cot_chain**：用户要改某条链的主题/信号分/正文，或删掉某条没价值的链时，先从 list_cot/search_memory 拿到那条的 `id`，再调 `edit_cot_chain(cot_id=..., set_tags/set_signal/set_trigger/set_cot/delete)`。改前用一句话说清改的是哪条、改成什么；delete 只删单条、会回显被删内容可恢复。tag 必须是闭合词表里的主题。
