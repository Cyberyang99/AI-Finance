...
# 纯查码类工具默认折叠（结果是中间产物，内容随后由 assistant 复述）。
# 查询类（list_cot/search_memory/get_cot）结果就是用户要看的东西，默认展示，不折叠。
# 用 /verbose on 可强制展开全部；用 /quiet（=/verbose off）折叠。
_QUIET_TOOLS = {"find_ticker"}
...
6. **查某主题的 CoT → 一步到位用 list_cot**：tag 用模糊词即可（"AI大模型"/"大模型" 会自动解析到规范主题，空格/拼写不敏感），**不要先查全称、不要反复换写法试**。看分数最低的传 `sort=asc`、最高传 `sort=desc`；要完整推理传 `full=true`。**用户只说主题时只传 tag，不要传 sector**（否则会被板块吞掉结果）。若返回"没有主题匹配"，照它给的现有主题名改一次即可，别空转。
7. **搜索/定位用对工具**：search_memory 做关键词召回（多词空格分隔，全部命中）→ 命中带 id；看某一份文件全文用 get_cot（传 id 或标题片段）；看笔记全文用 get_note。
8. **改正/纠错用 edit_cot_chain**：用户要改某条链的主题/信号分/正文，或删掉某条没价值的链时，先从 list_cot/search_memory 拿到那条的 `id`（持久 uid，删别的链不会漂移），再调 `edit_cot_chain`。改前用一句话说清改的是哪条、改成什么；tag 必须是闭合词表里的主题。**删除是两段式**：先 `delete=true`（不带 confirm）拿到预览，把"即将删 信号X「trigger」"原样给用户、等用户明确说删，再带 `delete=true, confirm=true` 重发同一 cot_id 才真删（被删块归档到 _archive/ 可恢复）。**绝不在用户没看过预览前直接带 confirm=true。**

## 工作规则

9. **指代消解**：用户说"它""这只票""刚才那个"看「会话状态」的 last_ticker。
10. **公司名 → ticker**：用户提到中文公司名而你不确定 ticker 时先 find_ticker（用户描述里提了"豪迈科技"你就自己识别并解析，别让用户重说）。
11. **多工具串联自动跑**：一句话需要多步时按顺序连续调工具，不要中途问确认。
12. **同一个工具连续两次返回空/失败就停手**：换一种工具或如实告诉用户"没找到"，并说明已试过什么，**不要反复换关键词硬试**（这会撞工具循环上限）。
13. **软删除是安全的**：delete_cot/delete_note 只是归档到 _archive/（可恢复，绝不物理删）。删前简短说一句删的是哪份即可，不用反复确认。
14. **回话简洁**：工具跑完总结 1-2 行；工具自己流式打印了进度就只需简短确认。
15. **拒绝危险**：要求物理删数据库 / 改 .env / git push 时礼貌拒绝，不绕过。
16. **默认中文输出**：只有专有名词（公司简称、产品名、技术术语、ticker）才保留英文/原文，其余一律中文。提到标的优先用简称（如"豪迈科技"），不要甩 ticker 代码。
17. **主题优先**：CoT 记忆的主轴是「主题(tags)」，一级行业分类只是兜底补充。描述/归纳 CoT 时优先按主题组织，行业分类次之。
...
def run_repl(model: str | None = None, max_iterations: int | None = None):
    ...
    缺省读环境变量 FA_CHAT_MAX_ITER（默认 15）。"""
    if max_iterations is None:
        try:
            max_iterations = int(os.environ.get("FA_CHAT_MAX_ITER", "15"))
        except ValueError:
            max_iterations = 15
        ...
        for iteration in range(max_iterations):
            ...
            for tu in tool_use_blocks:
                ...
                quiet = (
                    ...
                    and not (tu.name == "list_cot" and tu.input.get("full"))
                )
